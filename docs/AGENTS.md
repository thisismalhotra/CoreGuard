...
```
                          ┌─────────────────────────────────────────────┐
                          │                                             │
  Demand Data ──► Scout ──► Router ──► Solver ──► Buyer  │
                                                         │
  Shipment ──► Inspector ──────────────────────────────────┘
                                (on failure, triggers Buyer)
```
...
### Downstream
Hands off to **Router** when a spike is detected.

---

## 2. Router — Triage & Prioritisation Agent

**Source:** `backend/agents/dispatcher.py`
**Role:** Analyzes all BOM components for a spiked SKU and ranks them by urgency so Solver processes the most critical shortages first.

### Trigger
DEMAND_SPIKE event from Scout.

### Logic

For each BOM component, the Router calculates a priority score:
...
### Downstream
Hands the prioritised queue to **Solver**.

---

## 3. Solver — MRP Logic Agent
...
### Trigger
Prioritised queue from Router, or direct invocation from simulation endpoints.
...
When a component has a shortage, Solver checks if other finished goods using the same component have excess inventory:

```python
# Example: CH-231 is shared by FL-001-T and FL-001-S
# If FL-001-S has surplus, Solver can reallocate to cover FL-001-T's gap
```
...
Solver emits two types of actions:
...
### Downstream
BUY_ORDER actions are forwarded to **Buyer**.

---

## 4. Buyer — Procurement & PO Generation Agent
...
### Trigger
BUY_ORDER actions from Solver or Inspector.
...
- Footer: "Generated by Buyer Agent | Core-Guard MVP"
...
## 5. Inspector — Quality Inspection Agent
...
### Downstream
On failure, emits BUY_ORDER actions to **Buyer** for emergency reorder.
